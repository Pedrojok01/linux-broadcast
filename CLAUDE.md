...
```
   ...
   │  demand = consumers∪force_on∪gui_preview_active                    │
...
- `config.rs` — TOML config at `~/.config/linux-broadcast/config.toml` (`Mode`, `Model`, blur strength, source/sink, `start_on_login`, `force_on`).
